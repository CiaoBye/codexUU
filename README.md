...
当前预览版本：`0.2.01`。版本从 `0.0.1` 起步，每段累计 10 轮后进一位：`0.0.10` 的下一版为 `0.1.01`，`0.1.10` 的下一版为 `0.2.01`。每完成一轮可验收的代码更新都必须同步更新 `VERSION`、README、`AGENTS.md` 和 `agents/changelog.md`。
...
- 桌面状态悬浮窗：默认显示当前 Runtime 的可验证额度状态；提供信息圆盘、5h/7d 双环仪表、极简圆环、状态胶囊和双轨卡片五种样式。圆环统一从顶部起点绘制，已用向左、剩余向右；缺失窗口按需退化，支持小/中/大尺寸、主题同步、拖动和右键设置，点击中心可切换口径并同步主页面。
...
轻量模式下，托盘单击、全局快捷键和桌面悬浮窗双击都会恢复并前置主窗口；主窗口从最小化状态恢复时会同时调用 Qt 与 Windows 原生窗口恢复路径。
