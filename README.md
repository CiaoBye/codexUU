...
当前预览版本：`0.3.13`。版本从 `0.0.1` 起步，每段累计 10 轮后进一位：`0.0.10` 的下一版为 `0.1.01`，`0.1.10` 的下一版为 `0.2.01`。每完成一轮可验收的代码更新都必须同步更新 `VERSION`、README、`AGENTS.md` 和 `agents/changelog.md`。
...
- 顶栏提供“GPT / 全部”统计范围切换：GPT 口径只汇总精细日志中明确以 `gpt-` 开头的模型；全部口径包含可识别的第三方模型。切换同步作用于顶部四张 Token 卡、羊毛进度、趋势和模型使用量，并持久化选择。
