...
当前预览版本：`0.1.10`。版本从 `0.0.1` 起步，每段累计 10 轮后进一位：`0.0.10` 的下一版为 `0.1.01`，`0.1.10` 的下一版为 `0.2.01`。每完成一轮可验收的代码更新都必须同步更新 `VERSION`、README、`AGENTS.md` 和 `agents/changelog.md`。
...
- 桌面状态悬浮窗：默认显示当前 Runtime 的可验证额度状态；信息圆盘使用 270° 仪表弧，双环仪表为 5h 蓝色外环 + 7d 紫色内环，极简圆环只保留核心百分比。缺失窗口按需退化，支持小/中/大尺寸、主题同步、拖动和右键设置；点击圆心可在已用/剩余间切换并同步主页面。
