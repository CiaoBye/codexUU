...
5. **用量趋势与 0 基线图表**：
   - 方案 B 日期范围条与连续自然日热力图，缺失日期补零并显示 0–4 数值强度，累计范围最多展示最近 365 天；
...
7. **Skill 与真实工具调用**：
   - 仅统计显式 `function_call` / `custom_tool_call` 与 Skill 加载，按物理调用次数记录。

8. **Windows 原生集成**：
   - 单实例互斥防重开；
   - 系统托盘动态图标以状态环显示最新额度余量，tooltip 同步当前渠道、今日 Token 与额度百分比，并提供一键唤醒菜单；
