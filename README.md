...
当前预览版本：`0.1.07`。版本从 `0.0.1` 起步，每段累计 10 轮后进一位：`0.0.10` 的下一版为 `0.1.01`，`0.1.10` 的下一版为 `0.2.01`。每完成一轮可验收的代码更新都必须同步更新 `VERSION`、README、`AGENTS.md` 和 `agents/changelog.md`。
...
- 桌面状态悬浮窗：默认显示当前 Runtime 的今日 Token、5h / 7d 剩余额度与重置时间；可拖动、隐藏，并在设置或托盘菜单中开关。
- 轻量模式默认开启：主窗口运行时不显示 Windows 任务栏按钮，仍可通过通知区、全局快捷键和桌面状态窗唤回。
- 顶部今日 / 本周 / 本月 / 累计卡片可点击，直接跳转到对应口径的 Token 趋势；羊毛进度提供可关闭的轻量涌动提示。
...
- 通用：语言、Runtime、更新偏好、全局快捷键、置顶、轻量模式和关闭行为。
...
自动数据刷新固定为 60 秒，并使用 Windows/Qt 粗粒度定时器与刷新互斥。该周期优先控制 session 扫描和托盘重绘成本；手动刷新按钮仍可立即更新，但主界面不常驻显示自动刷新状态。设置页的语言、主题、Runtime、提醒阈值、快捷键和窗口行为均采用“选择后保存”机制；下拉框忽略鼠标滚轮，避免误触修改。

## Windows 发布

日常开发只推送代码和标签，不会自动构建或发布 Release。只有在 GitHub Actions 中手动运行 `Windows Release` 工作流时才会构建；可选择仅构建，或明确填写 Release 标签后发布安装包。工作流使用 PyInstaller 打包应用、使用 Inno Setup 生成当前用户范围的 Windows 安装器，并在发布时附带 `.exe` 与 SHA-256 校验文件。可在 Windows 本机执行：
