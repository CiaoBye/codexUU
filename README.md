...
当前预览版本：`0.3.22`。版本从 `0.0.1` 起步，每段累计 10 轮后进一位：`0.0.10` 的下一版为 `0.1.01`，`0.1.10` 的下一版为 `0.2.01`。每完成一轮可验收的代码更新都必须同步更新 `VERSION`、README、`AGENTS.md` 和 `agents/changelog.md`。

CodexUU 是一个面向 Windows 的本机 Codex 用量仪表盘，参考 [shanggqm/codexU](https://github.com/shanggqm/codexU) 的信息架构重新实现。

它读取本机 Codex 数据，集中展示额度窗口、Token 用量、API 等效价值、任务、趋势、项目排行、Skill 与工具调用。数据默认只在本机处理，不上传线程内容、项目路径或使用记录。

> CodexUU 是非官方项目，与 OpenAI、Anthropic 及上游 codexU 无隶属关系。

## 当前功能

- Codex 5 小时 / 7 天额度窗口，优先从独立 Codex runtime 实时读取；额度耗尽时仍展示后端返回的实际重置时间，缺失窗口按需隐藏且不伪造占位额度。
- 支持 CC Switch 当前 Codex 中转供应商的本机用量和余额显示：读取只读 `cc-switch.db` 的代理 Token/请求/成本，并按 CCS 已配置的用量查询脚本读取供应商余额；第三方余额与官方 Codex 额度和羊毛进度保持分离。
- 额度支持“剩余 / 已用”口径切换，并使用相反的环形进度方向表达。
- 今日、本周、本月、累计 Token，细分未缓存输入、缓存输入和输出。
- 按日志实际模型和公开单价估算 API 等效价值；OpenAI、DeepSeek 与 Xiaomi MiMo 只在模型 ID 精确匹配官方价格时计价，网关别名、内部模型和未知模型保持未计价并显示覆盖率。
- 今日项目看板：将同一 Runtime、同一项目的今日线程聚合为一张卡，展示进行中、待处理、定时和完成项目，避免按对话重复计数。
- 每日、每周、每月、累计趋势；每日视图包含半年 Token 活动热力图。
- 用量趋势内提供“概览 / 模型”二级视图，按真实日志中的模型与推理强度组合统计 Token、会话、回合、时间趋势和计价覆盖；模型排行、拆分指标与会话/回合数均跟随每日、每周、每月、累计口径，日志未提供强度时明确显示“未提供”。
- 项目用量排行与项目活动概览，支持本周、本月、累计口径。
- 点击项目可查看模型拆分、会话列表、筛选，并导出当前项目的 JSON / CSV 或复制 Markdown 摘要。
- Skill 使用与明确工具调用事件统计。
- 自动、浅色、深色主题和中英文界面。
- 主窗口普通尺寸保持统一设计比例并避免拖拽抖动；顶部范围按钮、Tab 和中英文标签按内容伸缩，设置页支持可调整大小、分类导航、内容滚动和固定保存栏。
- Windows 主窗口使用 Per-Monitor V2 DPI 感知；普通窗口的最小外框和 1060:720 客户区比例由 `WM_GETMINMAXINFO` / `WM_SIZING` 原生消息维护，最大化和全屏交由系统处理。
- 后台刷新对 rollout 解析结果做有界轻量缓存，仅保留统计所需元数据，避免大型 transcript 占用内存导致窗口假死。
- Windows 原生全局快捷键、主窗口置顶、关闭行为配置。
- Windows 11 动态托盘额度环与详细状态 tooltip；通知区图标会按当前 Runtime 和额度已用/剩余口径更新，单击直接恢复主窗口。
- 桌面状态悬浮窗：按定稿图提供信息圆盘 A、双环仪表 A、极简圆环 B、状态胶囊 B 和双轨卡片 B。信息圆盘使用双额度左右分栏，双环仪表使用纵向仪表与底部图例，极简圆环使用左环右数据分栏，胶囊使用额度标签组，双轨卡片使用今日用量与双轨分栏。所有圆环均保留完整灰色轨道；双额度时外环为紫色 7d、内环为蓝色 5h，单额度自动收敛且不保留空环。已用从底部沿左侧增长，剩余从底部沿右侧增长；支持小/中/大尺寸、主题同步、拖动和右键设置。按下立即反馈，单击动作在 220ms 判定后执行：中心切换口径、其他区域恢复主窗口；双击非拖动区域最小化主窗口。
- 顶栏提供“中转站点 / GPT / 全部”统计范围切换：中转站点标签使用 CCS 当前供应商名称并展示余额与本机代理用量，GPT 口径只汇总精细日志中明确以 `gpt-` 开头的模型，全部口径包含所有 Codex 本机模型。切换同步作用于顶部四张 Token 卡、趋势和模型使用量，并持久化选择；官方 5h/7d 额度、项目和任务保持各自数据口径。
...
- 数据源诊断：Codex app-server、SQLite 与 session 精细事件。
- CC Switch 中转用量：只读 `~/.cc-switch/cc-switch.db` 的代理日志，并按 CCS 已配置脚本查询当前供应商余额。
- 设置页提供 Codex 数据源诊断与刷新反馈。
...
| 数据 | 来源 |
|---|---|
| Codex 额度 | 优先通过独立 Codex runtime 的 `codex app-server` 读取当前 `codex` 额度桶，保留后端给出的重置时间；成功读取后最多复用 5 分钟，避免每分钟启动 Desktop 管理的 runtime。Windows Store 桌面版受进程启动限制且无独立 runtime 时，才读取最新 session 中官方写入的 `rate_limit` 快照（最多检查最近 32 个、14 天内的 rollout 文件）。后端明确返回空窗口且实时接口也没有时间时，仅标记为已用尽，不伪造过期重置时间 |
...
| 模型与推理强度 | Codex rollout 的 `turn_context` / `thread_settings_applied` 与后续 `token_count` 增量关联 |
| 模型价格 | OpenAI、DeepSeek、Xiaomi MiMo 官方公开按量价格；仅精确匹配模型 ID，第三方网关别名不推测 |
| 今日任务 | Codex SQLite 线程与启用中的 automation |
| 项目排行 | session 中的工作目录与精细 Token 增量，只保留仍存在的真实项目目录 |
| Skill 数据 | 明确的 Skill 加载，以及关联的 `function_call`、`custom_tool_call` 或对应 Runtime 显式事件 |
| CC Switch 中转用量 | `~/.cc-switch/cc-switch.db` 的 `proxy_request_logs` / `usage_daily_rollups`；余额使用当前供应商 `usage_script` 返回值 |
