...
当前预览版本：`0.2.04`。版本从 `0.0.1` 起步，每段累计 10 轮后进一位：`0.0.10` 的下一版为 `0.1.01`，`0.1.10` 的下一版为 `0.2.01`。每完成一轮可验收的代码更新都必须同步更新 `VERSION`、README、`AGENTS.md` 和 `agents/changelog.md`。
...
- 桌面状态悬浮窗：提供信息圆盘、双环仪表、极简圆环、状态胶囊和双轨卡片五种定稿样式。所有圆环均保留完整灰色轨道；双额度时外环为紫色 7d、内环为蓝色 5h，单额度自动收敛且不保留空环。已用从底部沿左侧增长，剩余从底部沿右侧增长；支持小/中/大尺寸、主题同步、拖动和右键设置。按下立即反馈，单击动作在 220ms 判定后执行：中心切换口径、其他区域恢复主窗口；双击非拖动区域最小化主窗口。
