...
from app.utils.rrule import compute_next_run
...
def _classify_thread_task(created_at, updated_at, recency_at, now):
    ...
    if statistics.date_for(activity) == today:
...
def _is_single_thread_cwd(cwd) -> bool:
    if not cwd:
        return True
    try:
        raw = str(cwd).replace("\\\\?\\", "").replace("\\\\?\\", "")
        p = Path(raw).resolve()
        if not p.exists():
            return False
        ...
        if p == home:
            return True
        temp = Path(os.environ.get("TMP", os.environ.get("TEMP", ""))).resolve()
        if temp.exists() and p == temp:
            return True
        if p.parent == p:
            return True
    ...
    if db_path:
        try:
            with _connect_state_db(db_path) as conn:
                ...
                order_fields = [name for name in ("recency_at", "updated_at", "created_at") if name in columns]
                ...
                rows = conn.execute(
                    "SELECT " + ", ".join((
                        ...
                        field("cwd", "''"), field("created_at"),
                        field("updated_at"), field("recency_at"),
                    )) + f" FROM threads ORDER BY {order_expr} DESC LIMIT 300"
                ).fetchall()
                now = datetime.now(timezone.utc)
                for tid, title, preview, cwd, created, updated, recency in rows:
                    classification = _classify_thread_task(
                        created, updated, recency, now,
                    )
                    if classification is None:
                        continue
                    status, activity_at = classification
                    single_thread = _is_single_thread_cwd(cwd)
                    project = "" if single_thread else Path(str(cwd).replace("\\\\?\\", "")).name if cwd else ""
                    tasks.append(TaskItem(
                        ...
                        project=project,
                        session_id=str(tid),
                        is_single_thread=single_thread,
                    ))
        except sqlite3.Error:
            pass
    ...
    if auto_dir.exists():
        for path in auto_dir.rglob("automation.toml"):
            try:
                ...
                rrule_match = re.search(r'rrule\s*=\s*["\'](.+?)["\']', content, re.DOTALL)
                rrule_text = rrule_match.group(1) if rrule_match else ""
                summary, next_run = compute_next_run(rrule_text) if rrule_text else ("", None)
                tasks.append(TaskItem(
                    id=str(path),
                    title=name_match.group(1) if name_match else path.parent.name,
                    status="scheduled",
                    runtime=RuntimeScope.CODEX,
                    detail=summary or ("Enabled automation" if enabled else "启用中的自动任务"),
