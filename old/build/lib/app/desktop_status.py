...
class DesktopStatusPanel(QWidget):
    ...
    def paintEvent(self, _event):
        ...
        surface = QColor("#151d2b" if dark else "#ffffff")
        edge = self._color("selection.stroke", "#354258" if dark else "#d6e0ee")
        track = self._color("data.zero", "#2c3749" if dark else "#e7ebf2")
        primary = self._color("quota.primary.start", "#8b72ff" if dark else "#705cf2")
        secondary = self._color("quota.secondary.start", "#44a2ff" if dark else "#3188e8")
        text = QColor("#f5f7fb" if dark else "#18243a")
        muted = QColor("#9cabc1" if dark else "#667995")
