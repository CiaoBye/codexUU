...
from app.utils.palette_manager import PaletteManager
...
from app.ui.palette_gallery import PaletteGalleryDialog
...
class SettingsDialog(QDialog):
    def __init__(self, parent=None, settings_manager: SettingsManager = None,
        ...
        self.palette_manager = getattr(self.theme_manager, "palette_manager", None) or PaletteManager()
    ...
    def _display_tab(self):
        ...
        saved_theme = self.theme_manager.get_theme()
        current_theme = saved_theme
        if saved_theme.startswith("palette."):
            current_theme = "dark"
            self._palette_id = saved_theme.split(".", maxsplit=1)[1]
        else:
            self._palette_id = "codexu.default"
        ...
        self._update_palette_btn_text()
        self.palette_btn.clicked.connect(self._open_palette_gallery)
        form.addRow("主题风格", self.palette_btn)
    ...
    def _open_palette_gallery(self):
        dialog = PaletteGalleryDialog(self.palette_manager, self._palette_id, self)
        dialog.palette_selected.connect(self._on_palette_selected)
        dialog.exec()

    def _on_palette_selected(self, palette_id):
        self._palette_id = palette_id
        self._update_palette_btn_text()
        self._mark_pending()

    def _update_palette_btn_text(self):
        definition = self.palette_manager.get(self._palette_id)
        if definition:
            label = definition.localized_name("zh-Hans")
        else:
            label = self._palette_id
        self.palette_btn.setText(label)
    ...
    def _apply_settings(self):
        ...
        if palette_id != "codexu.default":
            theme = f"palette.{palette_id}"
        ...
        if theme.startswith("palette."):
            self.settings_manager.set_palette(palette_id)
    ...
    def reject(self):
        ...
        controls = (
            ...
            (self.theme_combo, {"auto": 0, "light": 1, "dark": 2}.get(self.settings_manager.get_theme(), 2) if not self.settings_manager.get_theme().startswith("palette.") else 2),
        ...
        self._palette_id = (
            saved_theme.split(".", maxsplit=1)[1]
            if isinstance(saved_theme, str) and saved_theme.startswith("palette.")
            else "codexu.default"
        )
        self._update_palette_btn_text()
        mode, identifier = self.settings_manager.get_statistics_timezone()
    ...
    def _retranslate_ui(self):
        ...
        self.appearance_form.labelForField(self.palette_btn).setText("Theme style" if english else "主题风格")
        ...
        if current_theme.startswith("palette."):
            self.theme_combo.setCurrentIndex(2)
            self._palette_id = current_theme.split(".", maxsplit=1)[1]
        else:
            self.theme_combo.setCurrentIndex({"auto": 0, "light": 1, "dark": 2}.get(current_theme, 2))
            self._palette_id = "codexu.default"
