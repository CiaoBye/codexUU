    ...
    def paintEvent(self, _event):
        ...
        for index in sorted(range(len(widths)), key=widths.__getitem__, reverse=True):
            ...
            if deficit <= 0:
                break
        colors = (
            self._color("data.tokenInput", "#3f95ff"),
            self._color("data.tokenCached", "#8d74ff"),
            self._color("data.tokenOutput", "#e99a25"),
        )
        x = 0.0
...
class MetricCard(Surface):
    ...
    def __init__(self, label: str, icon: str, parent=None, theme_manager=None):
        ...
        self.composition = TokenCompositionBar(theme_manager=theme_manager)
        layout.addWidget(self.composition)
        self.breakdown_rows = {}
        breakdown = QVBoxLayout()
        breakdown.setSpacing(1)
        for key, token_path, label in (
            ("uncached", "data.tokenInput", "未缓存"),
            ("cached", "data.tokenCached", "缓存"),
            ("output", "data.tokenOutput", "输出"),
        ):
            fallback = {"uncached": "#3f95ff", "cached": "#8d74ff", "output": "#e99a25"}[key]
            color = self._color(token_path, fallback)
            row = QHBoxLayout()
            row.setSpacing(5)
            dot = QLabel()
    ...
    def set_theme_manager(self, theme_manager):
        self._theme_manager = theme_manager
        self.composition.set_theme_manager(theme_manager)

    def _color(self, token_path: str, fallback: str) -> str:
        if self._theme_manager:
            return self._theme_manager.get_palette_color(token_path, fallback)
        return fallback
...
class QuotaDial(QWidget):
    ...
    def paintEvent(self, event):
        ...
        primary = self._color("quota.primary.start", "#8d74ff")
        secondary = self._color("quota.secondary.start", "#3992ff")
        available = [
            item for item in (
                ("7d", self.q7, primary),
                ("5h", self.q5, secondary),
        ...
        entries = (("5H", self.q5, secondary, -47), ("7D", self.q7, primary, 13))
        for label, quota, color, offset in entries:
            ...
            painter.setPen(text_color)
            painter.setFont(QFont("Segoe UI Variable Display", 22, QFont.Weight.Bold))
            painter.drawText(
        ...
        painter.drawLine(QPointF(center.x() - 55, center.y() + 3), QPointF(center.x() + 55, center.y() + 5))
...
class QuotaResetStrip(QFrame):
    ...
    def __init__(self, parent=None, theme_manager=None):
        ...
        self.five_section, self.five_label, self.five_time = self._section("metric-today.svg", "quota.secondary.start", "#3992ff")
        self.seven_section, self.seven_label, self.seven_time = self._section("metric-week.svg", "quota.primary.start", "#8d74ff")
        self.divider = QFrame()
    ...
    def set_theme_manager(self, theme_manager):
        self._theme_manager = theme_manager
        self.five_label.setStyleSheet(f"color: {self._color('quota.secondary.start', '#3992ff')}; font-size: 9px; font-weight: 700;")
        self.seven_label.setStyleSheet(f"color: {self._color('quota.primary.start', '#8d74ff')}; font-size: 9px; font-weight: 700;")
    ...
    def _section(self, icon_name, token_path, fallback):
        section = QWidget()
        ...
        text.setSpacing(0)
        label = QLabel()
        label.setStyleSheet(f"color: {self._color(token_path, fallback)}; font-size: 9px; font-weight: 700;")
...
class QuotaPanel(Surface):
    ...
    def __init__(self, parent=None, theme_manager=None):
        ...
        header.addStretch()
        layout.addLayout(header)
        self.dial = QuotaDial(theme_manager=theme_manager)
        self.dial.center_activated.connect(self._toggle_center_mode)
        layout.addWidget(self.dial, 1)
        self.reset_strip = QuotaResetStrip(theme_manager=theme_manager)
    ...
    def set_theme_manager(self, theme_manager):
        self._theme_manager = theme_manager
        self.dial.set_theme_manager(theme_manager)
        self.reset_strip.set_theme_manager(theme_manager)
...
class MilestoneProgress(QWidget):
    ...
    def paintEvent(self, event):
        ...
        track = self._color("data.zero", "#7f91ac")
        ...
        milestone_colors = {
            "Plus": self._color("data.series.0", "#2596f3"),
            "Pro 100": self._color("data.series.1", "#8267e8"),
            "Pro 200": self._color("data.series.2", "#b25bd6"),
        }
        previous_x = left
        ...
        dark = bool(self._theme_manager and self._theme_manager.get_effective_theme() == "dark")
        dot_fill = QColor("#151d2b" if dark else "#ffffff")
        text_muted = QColor("#748197" if dark else "#8a94a6")
        for label, amount in self.MILESTONES:
            x = left + width * self.position(amount)
            painter.setBrush(QColor(dot_fill))
...
class ValueCard(Surface):
    def __init__(self, parent=None, theme_manager=None):
        ...
        layout.addLayout(header)
        self.bar = MilestoneProgress(theme_manager=theme_manager)
    ...
    def set_theme_manager(self, theme_manager):
        if hasattr(self, "bar"):
            self.bar.set_theme_manager(theme_manager)
        self.update()
...
class DashboardWidget(QWidget):
    ...
    def _build_summary(self):
        ...
        layout.setSpacing(10)
        self.quota_card = QuotaPanel(theme_manager=self.theme_manager)
        ...
        cards = QHBoxLayout()
        cards.setSpacing(10)
        self.today_card = MetricCard("今日", "metric-today.svg", theme_manager=self.theme_manager)
        self.week_card = MetricCard("本周", "metric-week.svg", theme_manager=self.theme_manager)
        self.month_card = MetricCard("本月", "metric-month.svg", theme_manager=self.theme_manager)
        self.cumulative_card = MetricCard("累计", "metric-all.svg", theme_manager=self.theme_manager)
        for card, mode in (
        ...
        self.value_card = ValueCard(theme_manager=self.theme_manager)
    ...
    def _build_tab_area(self):
        ...
        self.trend_tab = UsageTrendWidget(theme_manager=self.theme_manager)
    ...
    def _on_theme_changed(self):
        if self.theme_manager:
            self.quota_card.set_theme_manager(self.theme_manager)
            self.today_card.set_theme_manager(self.theme_manager)
            self.week_card.set_theme_manager(self.theme_manager)
            self.month_card.set_theme_manager(self.theme_manager)
            self.cumulative_card.set_theme_manager(self.theme_manager)
            self.value_card.set_theme_manager(self.theme_manager)
            self.trend_tab.set_theme_manager(self.theme_manager)
    ...
    def _check_worker(self):
        if self._pending_result is not None:
            ...
            self._update()
            now = get_statistics_timezone().datetime_for(datetime.now(timezone.utc)).strftime("%H:%M:%S")
