...
class SettingsManager:
    ...
    def set_theme(self, theme: str):
        if not isinstance(theme, str):
            return
        value = theme.strip()
        if value in ("auto", "light", "dark") or (value.startswith("palette.") and len(value) > len("palette.")):
            self.theme = value
            self._notify_listeners()
    ...
    def set_palette(self, palette_id: str):
        if palette_id:
            ...
            self.theme = f"palette.{palette_id}"
            self._notify_listeners()
    ...
    def load(self):
        if self.config_path.exists():
            try:
                ...
                self.palette = palette.strip() if isinstance(palette, str) and palette.strip() else DEFAULT_PALETTE
    ...
    def _normalize_theme(theme) -> str:
        ...
        if value in ("auto", "light", "dark"):
            return value
        if value.startswith("palette.") and len(value) > len("palette."):
            return value
        return DEFAULT_THEME
